...
DEFAULT_SRC_MAC = "a0:36:9f:0a:5d:5b"
DEFAULT_SRC_IP = "10.0.16.3"
DEFAULT_DST_IP = "10.0.18.3"
...
SAMPLE_POOL_MODE_TRUNCATE = "truncate"
SAMPLE_POOL_MODE_REPEAT = "repeat"
EXPERIMENT_MACS = [
    "00:4e:46:32:43:00",
    "00:4e:46:32:43:01",
    "00:4e:46:32:43:02",
...
def _select_samples(vectors, expected_rows, limit, sample_pool_mode):
    if len(vectors) != len(expected_rows):
        raise SystemExit(
            "test vector / expected output length mismatch (%d != %d)"
            % (len(vectors), len(expected_rows))
        )

    source_count = len(vectors)
    mode = str(sample_pool_mode or SAMPLE_POOL_MODE_TRUNCATE).strip().lower()
    if mode not in {SAMPLE_POOL_MODE_TRUNCATE, SAMPLE_POOL_MODE_REPEAT}:
        raise SystemExit("unsupported sample_pool_mode: %s" % sample_pool_mode)

    if limit is None:
        selected_pairs = [(index, vectors[index], expected_rows[index]) for index in range(len(vectors))]
    else:
        ...
        if source_count == 0:
            selected_pairs = []
        elif mode == SAMPLE_POOL_MODE_TRUNCATE or target_count <= source_count:
            selected_pairs = [
                (index, vectors[index], expected_rows[index])
                for index in range(min(target_count, source_count))
            ]
        else:
            selected_pairs = []
            for index in range(target_count):
                source_index = index % source_count
                selected_pairs.append((source_index, vectors[source_index], expected_rows[source_index]))

    selected_vectors = []
    selected_expected_rows = []
    for source_index, vector, expected_row in selected_pairs:
        vector_copy = copy.deepcopy(vector)
        expected_copy = copy.deepcopy(expected_row)
        vector_copy["source_name"] = str(vector_copy.get("source_name") or vector_copy.get("name") or ("sample%d" % source_index))
        vector_copy["source_sample_index"] = int(vector_copy.get("source_sample_index", source_index))
        expected_copy["source_name"] = str(expected_copy.get("source_name") or expected_copy.get("name") or ("sample%d" % source_index))
        expected_copy["source_sample_index"] = int(expected_copy.get("source_sample_index", source_index))
        selected_vectors.append(vector_copy)
        selected_expected_rows.append(expected_copy)
    prepared_count = len(selected_vectors)
    unique_count = min(source_count, prepared_count)
    counts = {
        "source_test_vector_count": source_count,
        "prepared_sample_count": prepared_count,
        "sample_pool_mode": mode,
        "sample_pool_unique_count": unique_count,
        "sample_pool_repeated": prepared_count > unique_count,
    }
    return selected_vectors, selected_expected_rows, counts


def _annotate_prepared_samples(vectors, expected_rows, request_id_base):
    annotated_vectors = []
    annotated_expected_rows = []
    for prepared_index, (vector, expected_row) in enumerate(zip(vectors, expected_rows)):
        request_id = (int(request_id_base) + prepared_index) & 0xFFFF
        source_name = str(vector.get("source_name") or vector.get("name") or ("sample%d" % prepared_index))
        source_sample_index = int(vector.get("source_sample_index", prepared_index))
        prepared_label = "%s@0x%04x" % (source_name, request_id)

        vector_row = copy.deepcopy(vector)
        vector_row["source_name"] = source_name
        vector_row["source_sample_index"] = source_sample_index
        vector_row["prepared_index"] = prepared_index
        vector_row["prepared_label"] = prepared_label
        vector_row["request_id"] = request_id
        annotated_vectors.append(vector_row)

        expected_out = copy.deepcopy(expected_row)
        expected_out["source_name"] = str(expected_out.get("source_name") or source_name)
        expected_out["source_sample_index"] = int(expected_out.get("source_sample_index", source_sample_index))
        expected_out["prepared_index"] = prepared_index
        expected_out["prepared_label"] = prepared_label
        expected_out["request_id"] = request_id
        annotated_expected_rows.append(expected_out)
    return annotated_vectors, annotated_expected_rows
...
def _build_manifest(run_dir, bundle_result, vectors, expected_rows, pcap_info, args, sample_counts):
    run_name = Path(run_dir).name
    remote_netfpga_root = "/home/netfpga/scripts/%s/%s_netfpga" % (DEFAULT_REMOTE_VERSION, run_name)
    remote_netfpga_results = "/home/netfpga/scripts/%s/%s_results" % (DEFAULT_REMOTE_VERSION, run_name)
    ...
    manifest = {
        ...
        "counts": {
            ...
            "prepared_sample_count": int(sample_counts["prepared_sample_count"]),
            "source_test_vector_count": int(sample_counts["source_test_vector_count"]),
            "sample_pool_mode": str(sample_counts["sample_pool_mode"]),
            "sample_pool_unique_count": int(sample_counts["sample_pool_unique_count"]),
            "sample_pool_repeated": bool(sample_counts["sample_pool_repeated"]),
...
def _missing_request_ids(summary, expected_rows):
    if summary.get("missing_request_ids") is not None:
        return [item for item in summary.get("missing_request_ids", []) if item]
    expected_by_name = {row.get("name"): row for row in expected_rows}
    missing = []
    for name in summary.get("missing_samples", []):
...
def _augment_batch_summary(summary, manifest, expected_rows, observed_rows, batch_capture_path, debug_status_text=None):
    ...
    summary["missing_request_ids"] = _missing_request_ids(summary, expected_rows)
    if debug_status:
    ...
    summary["correctness_verdict"] = "healthy"
    if summary.get("mismatches") or summary.get("missing_samples"):
        summary["correctness_verdict"] = "mismatch"
    return summary
...
def _render_capture_workflow(run_dir, manifest):
    ...
    batch_time_window_seconds = float(
        manifest.get("capture", {}).get("batch_time_window_seconds", DEFAULT_BATCH_CAPTURE_TIME_WINDOW_SECONDS)
    )
    ...
    sender_batch_capture_primary = _remote_shell_path("%s/captures/offload_batch_sender_primary.cap" % usc["remote_sender_root"])
    sender_batch_capture_fallback = _remote_shell_path("%s/captures/offload_batch_sender_fallback.cap" % usc["remote_sender_root"])
    batch_capture_time_window = _remote_shell_path("%s/%s" % (
        usc["remote_receiver_root"],
        artifacts["offload_batch_time_window_capture"],
    ))
    batch_capture_primary = _remote_shell_path("%s/captures/offload_batch_receiver_primary.cap" % usc["remote_receiver_root"])
    batch_capture_fallback = _remote_shell_path("%s/captures/offload_batch_receiver_fallback.cap" % usc["remote_receiver_root"])
    offload_smoke_capture = _remote_shell_path("%s/%s" % (
        usc["remote_receiver_root"],
        artifacts["offload_smoke_capture"],
    ))
...
""".format(
        ...
        "nf4_capture_offload_batch_sender_primary.sh": """#!/usr/bin/env bash
set -euo pipefail
mkdir -p "{capture_dir}"
rm -f "{capture_path}"
sudo /usr/sbin/tcpdump -i {iface} -nn -U -c {count} '{capture_filter}' -w "{capture_path}"
""".format(
            ...
            count=batch_count,
            capture_filter=capture_filter,
            capture_path=sender_batch_capture_primary,
        ),
        "nf4_capture_offload_batch_sender_fallback.sh": """#!/usr/bin/env bash
set -euo pipefail
mkdir -p "{capture_dir}"
rm -f "{capture_path}"
sudo /usr/sbin/tcpdump -i {iface} -nn -U -c {count} '{capture_filter}' -w "{capture_path}"
""".format(
            ...
            count=batch_count,
            capture_filter=capture_filter,
            capture_path=sender_batch_capture_fallback,
        ),
        "nf4_cleanup_offload_batch_sender_captures.sh": """#!/usr/bin/env bash
set -euo pipefail
rm -f "{canonical_path}" "{primary_path}" "{fallback_path}"
""".format(
            canonical_path=sender_batch_capture,
            primary_path=sender_batch_capture_primary,
            fallback_path=sender_batch_capture_fallback,
        ),
        "nf1_cleanup_offload_batch_receiver_captures.sh": """#!/usr/bin/env bash
set -euo pipefail
for capture_path in "{canonical_path}" "{primary_path}" "{fallback_path}"; do
  rm -f "$capture_path"
...
""".format(
            canonical_path=batch_capture_time_window,
            primary_path=batch_capture_primary,
            fallback_path=batch_capture_fallback,
        ),
        "nf1_capture_offload_batch_count.sh": """#!/usr/bin/env bash
set -euo pipefail
mkdir -p "{capture_dir}"
rm -f "{capture_path}"
sudo /usr/sbin/tcpdump -i {iface} -nn -U -c {count} '{capture_filter}' -w "{capture_path}"
""".format(
            ...
            count=batch_count,
            capture_filter=capture_filter,
            capture_path=batch_capture_primary,
        ),
...
""".format(
            ...
            iface=usc["receiver_iface"],
            capture_filter=capture_filter,
            capture_path=batch_capture_fallback,
        ),
...
""".format(
        ...
        "nf1_capture_offload_smoke.sh": """#!/usr/bin/env bash
set -euo pipefail
mkdir -p "{capture_dir}"
rm -f "{capture_path}"
sudo /usr/sbin/tcpdump -i {iface} -nn -U -c 1 '{capture_filter}' -w "{capture_path}"
""".format(
            ...
            iface=usc["receiver_iface"],
            capture_filter=capture_filter,
            capture_path=offload_smoke_capture,
        ),
...
""".format(
        ...
        "nf4_replay_offload_smoke.sh": """#!/usr/bin/env bash
set -euo pipefail
sudo /usr/bin/tcpreplay -i {iface} "{pcap_path}"
""".format(
            iface=usc["sender_iface"],
            pcap_path="%s/%s" % (sender_pcap_root, Path(artifacts["offload_smoke_pcap"]).name),
        ),
...
def _render_markdown_summary(manifest, batch_summary, wrong_magic_row, wrong_port_row, debug_status_text=None):
    ...
    if "correctness_verdict" in batch_summary:
        lines.append("- correctness_verdict: `%s`" % batch_summary.get("correctness_verdict"))
...
def prepare_cmd(args):
    ...
    source_vectors = load_test_vectors(bundle_result.test_vector_path)
    source_expected_rows = load_expected_outputs(bundle_result.expected_output_path)
    vectors, expected_rows, sample_counts = _select_samples(
        source_vectors,
        source_expected_rows,
        args.limit,
        getattr(args, "sample_pool_mode", SAMPLE_POOL_MODE_TRUNCATE),
    )
    vectors, expected_rows = _annotate_prepared_samples(
        vectors,
        expected_rows,
        _parse_int(args.request_id_base),
    )
    ...
    manifest = _build_manifest(run_dir, bundle_result, vectors, expected_rows, pcap_info, args, sample_counts)
    ...
    print("source_test_vector_count=%d" % sample_counts["source_test_vector_count"])
    print("prepared_sample_count=%d" % sample_counts["prepared_sample_count"])
    print("sample_pool_mode=%s" % sample_counts["sample_pool_mode"])
...
def build_parser():
    ...
    prepare.add_argument(
        "--sample-pool-mode",
        choices=[SAMPLE_POOL_MODE_TRUNCATE, SAMPLE_POOL_MODE_REPEAT],
        default=SAMPLE_POOL_MODE_TRUNCATE,
    )
