...
class BoardDebugUdpPacketTests(unittest.TestCase):
    def test_boardctl_select_samples_repeat_expands_to_limit(self) -> None:
        vectors = [{"name": "v0"}, {"name": "v1"}]
        expected_rows = [{"name": "e0"}, {"name": "e1"}]

        selected_vectors, selected_expected_rows, counts = boardctl._select_samples(
            vectors,
            expected_rows,
            5,
            boardctl.SAMPLE_POOL_MODE_REPEAT,
        )

        self.assertEqual(len(selected_vectors), 5)
        self.assertEqual(len(selected_expected_rows), 5)
        self.assertEqual([item["name"] for item in selected_vectors], ["v0", "v1", "v0", "v1", "v0"])
        self.assertEqual([item["name"] for item in selected_expected_rows], ["e0", "e1", "e0", "e1", "e0"])
        self.assertEqual([item["source_sample_index"] for item in selected_vectors], [0, 1, 0, 1, 0])
        self.assertEqual([item["source_sample_index"] for item in selected_expected_rows], [0, 1, 0, 1, 0])
        self.assertEqual(counts["source_test_vector_count"], 2)
        self.assertEqual(counts["prepared_sample_count"], 5)
        self.assertEqual(counts["sample_pool_unique_count"], 2)
        self.assertTrue(counts["sample_pool_repeated"])

    def test_boardctl_annotate_prepared_samples_assigns_request_ids_and_labels(self) -> None:
        vectors = [{"name": "v0"}, {"name": "v1"}, {"name": "v0"}]
        expected_rows = [{"name": "e0"}, {"name": "e1"}, {"name": "e0"}]

        annotated_vectors, annotated_expected = boardctl._annotate_prepared_samples(vectors, expected_rows, 0x2200)

        self.assertEqual([item["request_id"] for item in annotated_vectors], [0x2200, 0x2201, 0x2202])
        self.assertEqual([item["prepared_index"] for item in annotated_vectors], [0, 1, 2])
        self.assertEqual([item["source_sample_index"] for item in annotated_vectors], [0, 1, 2])
        self.assertEqual(annotated_vectors[0]["prepared_label"], "v0@0x2200")
        self.assertEqual(annotated_vectors[2]["prepared_label"], "v0@0x2202")
        self.assertEqual([item["request_id"] for item in annotated_expected], [0x2200, 0x2201, 0x2202])
        self.assertEqual(annotated_expected[2]["prepared_label"], "v0@0x2202")

    def test_boardctl_select_samples_truncate_preserves_limit(self) -> None:
        vectors = [{"name": "v0"}, {"name": "v1"}, {"name": "v2"}]
        expected_rows = [{"name": "e0"}, {"name": "e1"}, {"name": "e2"}]

        selected_vectors, selected_expected_rows, counts = boardctl._select_samples(
            vectors,
            expected_rows,
            2,
            boardctl.SAMPLE_POOL_MODE_TRUNCATE,
        )

        self.assertEqual([item["name"] for item in selected_vectors], ["v0", "v1"])
        self.assertEqual([item["name"] for item in selected_expected_rows], ["e0", "e1"])
        self.assertEqual(counts["prepared_sample_count"], 2)
        self.assertEqual(counts["sample_pool_mode"], boardctl.SAMPLE_POOL_MODE_TRUNCATE)
        self.assertFalse(counts["sample_pool_repeated"])
    ...
    def test_udp_frame_defaults_emit_udp_unknown_packet(self) -> None:
        frame, metadata = build_udp_frame_defaults(payload=b"DEMO_BYPASS", udp_dst_port=0x7777)
        parsed = inspect_ann_frame(frame)

        self.assertEqual(metadata["ethertype"], "0x0800")
        self.assertEqual(metadata["ip_protocol"], "0x11")
        self.assertEqual(metadata["udp_dst_port"], "0x7777")
        self.assertEqual(metadata["payload_hex"], b"DEMO_BYPASS".hex())
        self.assertEqual(parsed["frame_kind"], "udp_unknown")
        self.assertEqual(parsed["udp_dst_port"], "0x7777")
        self.assertEqual(parsed["payload_len"], len(b"DEMO_BYPASS"))
    ...
    def test_compare_expected_observed_uses_request_id_for_repeated_names(self) -> None:
        expected_rows = [
            {
                "name": "window_id_20",
                "prepared_label": "window_id_20@0x2200",
                "prepared_index": 0,
                "request_id": 0x2200,
                "predicted_class": 1,
                "wire_result_data_0_u16": 1,
            ...
            {
                "name": "window_id_20",
                "prepared_label": "window_id_20@0x2201",
                "prepared_index": 1,
                "request_id": 0x2201,
                "predicted_class": 1,
                "wire_result_data_0_u16": 1,
        ...
        self.assertEqual(summary["class_matches"], 2)
        self.assertEqual(summary["wire_matches"], 2)
        self.assertEqual(summary["missing_samples"], [])
        self.assertEqual(summary["missing_request_ids"], [])
    ...
    def test_boardctl_augments_report_with_debug_emit_gap(self) -> None:
        ...
        self.assertEqual(augmented["correctness_verdict"], "mismatch")
