...
def load_test_vectors(path):
    ...
    while index < len(raw):
        ...
        while value_index < len(values):
            ...
            value_index += 1
        row = {"name": name, "input_s16": parsed_values}
        if entry.get("source_name") is not None:
            row["source_name"] = str(entry.get("source_name"))
        if entry.get("prepared_label") is not None:
            row["prepared_label"] = str(entry.get("prepared_label"))
        if entry.get("prepared_index") is not None:
            row["prepared_index"] = int(entry.get("prepared_index"))
        if entry.get("source_sample_index") is not None:
            row["source_sample_index"] = int(entry.get("source_sample_index"))
        if entry.get("request_id") is not None:
            row["request_id"] = int(str(entry.get("request_id")), 0)
        vectors.append(row)
        index += 1
    return vectors
...
def _row_display_name(row, fallback_index):
    if row.get("prepared_label") is not None:
        return str(row["prepared_label"])
    if row.get("request_id") is not None:
        return "%s@0x%04x" % (str(row.get("name", "sample%d" % fallback_index)), int(row["request_id"]) & 0xFFFF)
    return str(row.get("name", "sample%d" % fallback_index))


def compare_expected_observed(expected_rows, observed_rows):
    use_request_id_alignment = True
    for row in expected_rows:
        if row.get("request_id") is None:
            use_request_id_alignment = False
            break

    if not use_request_id_alignment:
        expected_by_name = {}
        observed_by_name = {}
        names = []
        ...
        for name in names:
            ...
            if observed is None:
                ...
                if expected.get("request_id") is not None:
                    ...
                    missing_request_ids.append("0x%04x" % int(expected["request_id"]))
    ...
    expected_order = []
    observed_by_request_id = {}

    for index, row in enumerate(expected_rows):
        request_id = int(row["request_id"])
        expected_by_request_id[request_id] = row
        expected_order.append(request_id)

    for index, row in enumerate(observed_rows):
        request_id = row.get("request_id")
        if request_id is None:
            request_id = index
        observed_by_request_id[int(request_id)] = row
    ...
    for index, request_id in enumerate(expected_order):
        expected = expected_by_request_id[request_id]
        observed = observed_by_request_id.get(request_id)
        display_name = _row_display_name(expected, index)
        if observed is None:
            missing_samples.append(display_name)
            missing_request_ids.append("0x%04x" % request_id)
            mismatch = {"name": display_name, "reason": "missing_observation"}
            if expected.get("request_id") is not None:
                mismatch["expected_request_id"] = "0x%04x" % int(expected["request_id"])
            mismatches.append(mismatch)
            continue
...
def build_batch_frame_rows(
    ...
    while index < len(test_vectors):
        ...
        if vector.get("prepared_label") is not None:
            row["prepared_label"] = str(vector["prepared_label"])
        if vector.get("prepared_index") is not None:
            row["prepared_index"] = int(vector["prepared_index"])
        if vector.get("source_sample_index") is not None:
            row["source_sample_index"] = int(vector["source_sample_index"])
        rows.append(row)
        index += 1
    return rows
...
def observed_rows_from_frames(
    ...
    while index < len(frames):
        ...
        if expected is not None:
            ...
            if expected.get("prepared_label") is not None:
                row["prepared_label"] = str(expected["prepared_label"])
            if expected.get("prepared_index") is not None:
                row["prepared_index"] = int(expected["prepared_index"])
            if expected.get("source_sample_index") is not None:
                row["source_sample_index"] = int(expected["source_sample_index"])
