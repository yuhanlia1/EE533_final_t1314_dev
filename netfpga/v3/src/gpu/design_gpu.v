...
module gpu_top_fifo_if #(
  ...
  assign stall_pipe  = stall_ex | stall_mm;
...
module gpu_ex_mm_reg #(
  ...
  reg stall_prev;
  wire stall_release = stall_prev & ~stall;
...
module gpu_mm_stage #(
  ...
  reg         load_pending;
  reg [15:0]  load_ctrl_r;
  reg [2:0]   load_rd_r;
  reg [1:0]   load_lane_mask_r;
  reg [63:0]  load_res0_r;
  reg [63:0]  load_res1_r;

  wire is_load  = in_ctrl[0];
  wire is_store = in_ctrl[1];

  wire issue_load  = in_valid & proc_active & is_load  & ~load_pending;
  wire issue_store = in_valid & proc_active & is_store;

  wire load_rsp_done = load_pending &&
                       ((!load_lane_mask_r[0] || mem0_rvalid) &&
                        (!load_lane_mask_r[1] || mem1_rvalid));

  assign consume_pulse = in_valid & (((~is_load) & (~is_store)) | issue_store | issue_load);
  ...
  assign out_ctrl      = load_rsp_done ? load_ctrl_r      : in_ctrl;
  assign out_rd        = load_rsp_done ? load_rd_r        : in_rd;
  assign out_lane_mask = load_rsp_done ? load_lane_mask_r : in_lane_mask;
  assign out_res0      = load_rsp_done ? load_res0_r      : in_res0;
  assign out_res1      = load_rsp_done ? load_res1_r      : in_res1;
