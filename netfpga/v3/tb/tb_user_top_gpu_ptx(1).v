...
module tb_user_top_gpu_ptx_v2lane;
  ...
  reg prev_cpu_proc_active;
  reg prev_gpu_proc_active;
  reg prev_pkt_ready;
  reg prev_gpu_done;
  ...
  function [63:0] pack4;
    input [15:0] x0; input [15:0] x1; input [15:0] x2; input [15:0] x3;
    begin pack4 = {x3, x2, x1, x0}; end
  endfunction
  ...
  function [31:0] inst_enc;
    ...
    input        dtype;
    input [15:0] imm;
    begin inst_enc = {opcode, rd, rs1, rs2, bsel, dtype, imm}; end
  endfunction
  ...
  task load_programs_v2;
    ...
    begin
      ...
      dut.u_gpu.u_if.u_imem.mem[a+14] = inst_enc(4'h3, 3'd0, 3'd0, 3'd2, 2'd2, 1'b0, 16'd0);
  ...
  task clear_expected; integer j; begin for (j = 0; j < 8; j = j + 1) expected[j] = 64'd0; end endtask
  task clear_pkt_mem; integer j; begin for (j = 0; j < 256; j = j + 1) pkt_mem[j] = 64'd0; end endtask
  task clear_outputs; integer j; begin out_count = 0; for (j = 0; j < 256; j = j + 1) out_cap[j] = 72'd0; end endtask

  task pkt_write_global;
    input integer base; input integer idx; input [63:0] data;
    begin pkt_mem[base + idx] = data; end
  endtask

  task fill_output_sentinel;
    input integer base; integer j;
    begin for (j = 0; j < 4; j = j + 1) pkt_mem[base + 2 + j] = SENTINEL; end
  endtask

  task gpu_mmio_write_force;
    input [7:0] addr; input [31:0] data;
    begin
      @(negedge clk);
  ...
  task gpu_prepare_kernel;
    input [8:0]  entry_pc; input [31:0] work_size;
    input [31:0] base_a; input [31:0] base_b; input [31:0] base_c; input [31:0] base_d;
    begin
      ...
      gpu_mmio_write_force(REG_BASE_A_LO, base_a); gpu_mmio_write_force(REG_BASE_A_HI, 32'd0);
      gpu_mmio_write_force(REG_BASE_B_LO, base_b); gpu_mmio_write_force(REG_BASE_B_HI, 32'd0);
      gpu_mmio_write_force(REG_BASE_C_LO, base_c); gpu_mmio_write_force(REG_BASE_C_HI, 32'd0);
      gpu_mmio_write_force(REG_BASE_D_LO, base_d); gpu_mmio_write_force(REG_BASE_D_HI, 32'd0);
      gpu_mmio_write_force(REG_M, 32'd0); gpu_mmio_write_force(REG_N, 32'd0); gpu_mmio_write_force(REG_K, 32'd0);
    end
  endtask

  task gpu_start_kernel; begin gpu_mmio_write_force(REG_CONTROL, 32'h00000001); end endtask
  ...
  task print_packet_preview;
    input [127:0] name; input integer total_words; integer j;
    begin
  ...
  task send_packet_words;
    input integer nwords; integer j;
    begin
      ...
      @(negedge clk);
      in_wr = 1'b0; in_ctrl = 8'h00; in_data = 64'd0;
    end
  endtask

  task wait_for_proc_active;
    input integer limit; integer k; reg ok;
    begin
      ok = 1'b0;
      for (k = 0; k < limit; k = k + 1) begin
        @(posedge clk);
        if (dut.u_fifo.gpu_proc_active) begin ok = 1'b1; k = limit; end
      end
      if (!ok) begin
  ...
  task wait_gpu_done;
    ...
    begin
      ...
      for (k = 0; k < limit; k = k + 1) begin
        ...
        if (dut.gpu_proc_done) begin ok = 1'b1; k = limit; end
      end
      if (!ok) begin
    ...
    begin
      ...
      for (k = 0; k < limit; k = k + 1) begin
        ...
        if (dut.u_fifo.hold_valid == 1'b0 &&
            ...
            dut.u_fifo.proc_active == 1'b0 &&
            dut.u_fifo.gpu_proc_active == 1'b0) begin ok = 1'b1; k = limit; end
      end
      if (!ok) begin
  ...
  task setup_add_case;
    begin
      clear_pkt_mem(); clear_expected(); fill_output_sentinel(BASE_C);
  ...
  task setup_relu_case;
    begin
      clear_pkt_mem(); clear_expected(); fill_output_sentinel(BASE_B);
  ...
  task setup_fma_case;
    begin
      clear_pkt_mem(); clear_expected(); fill_output_sentinel(BASE_D);
      pkt_write_global(BASE_A, 0, pack4(BF16_1,   BF16_2,   BF16_3,   BF16_4));
      pkt_write_global(BASE_B, 0, pack4(BF16_5,   BF16_6,   BF16_7,   BF16_8));
  ...
  task run_packet_kernel;
    ...
    begin
      ...
      wait_gpu_done(12000);
      wait_for_idle(2000);
      check_packet_region(name, verify_base, 2, total_words);
  ...
  always @(posedge clk) begin
    if (reset) begin
      prev_df_state <= 2'b11;
      prev_cpu_proc_active <= 1'b0;
      prev_gpu_proc_active <= 1'b0;
      prev_pkt_ready <= 1'b0;
      prev_gpu_done <= 1'b0;
    end else begin
      if (prev_df_state != dut.u_fifo.drop_fifo.state)
        $display("[%0t] DFSM %0d -> %0d", $time, prev_df_state, dut.u_fifo.drop_fifo.state);
      if (prev_cpu_proc_active != dut.u_fifo.proc_active)
        $display("[%0t] cpu_proc_active -> %b", $time, dut.u_fifo.proc_active);
      if (prev_gpu_proc_active != dut.u_fifo.gpu_proc_active)
        $display("[%0t] gpu_proc_active -> %b", $time, dut.u_fifo.gpu_proc_active);
      ...
      prev_cpu_proc_active <= dut.u_fifo.proc_active;
      prev_gpu_proc_active <= dut.u_fifo.gpu_proc_active;
  ...
  always @(posedge clk) begin
    if (!reset && (dut.gpu_mem0_en || dut.gpu_mem1_en)) begin
      $display("[%0t] GPU MEM m0_en=%b we=%b addr=%0d wdata=%h rvalid=%b rdata=%h | m1_en=%b we=%b addr=%0d wdata=%h rvalid=%b rdata=%h",
        $time,
        dut.gpu_mem0_en,
        dut.gpu_mem0_we,
        dut.gpu_mem0_addr,
  ...
  initial begin
    ...
    $dumpfile("tb_user_top_gpu_ptx_v2lane.vcd");
    $dumpvars(0, tb_user_top_gpu_ptx_v2lane);

    setup_add_case;
    run_packet_kernel("vec_add_i16x4_2lane", ENTRY_ADD, 32'd2, BASE_A, BASE_B, BASE_C, 32'd0, BASE_C + 6, BASE_C);

    setup_sub_case;
    run_packet_kernel("vec_sub_i16x4_2lane", ENTRY_SUB, 32'd2, BASE_A, BASE_B, BASE_C, 32'd0, BASE_C + 6, BASE_C);

    setup_relu_case;
    run_packet_kernel("relu_i16x4_2lane", ENTRY_RELU, 32'd2, BASE_A, 32'd0, BASE_B, 32'd0, BASE_B + 6, BASE_B);

    setup_mul_case;
    run_packet_kernel("vec_mul_bf16x4_2lane", ENTRY_MUL, 32'd2, BASE_A, BASE_B, BASE_C, 32'd0, BASE_C + 6, BASE_C);

    setup_fma_case;
    run_packet_kernel("fma_bf16x4_2lane", ENTRY_FMA, 32'd2, BASE_A, BASE_B, BASE_C, BASE_D, BASE_D + 6, BASE_D);
