...
my $DEFINES  = File::Spec->catfile($ROOT_DIR, 'sw', 'reg_defines_v7.h');
...
copy(
  $DEFINES,
  File::Spec->catfile($temp_root, 'sw', 'reg_defines_v7.h'),
) or die "failed to copy reg_defines_v7.h: $!";

my $temp_ctl = new_ctl($env, root_dir => $temp_root, state_dir => File::Spec->catdir($temp_root, 'state'));
is($temp_ctl->resolve_register('sw_gpu_ofmap_addr')->{addr}, $ADDR{sw_gpu_ofmap_addr}, 'register map loads from sw/reg_defines_v7.h under the selected root');

my $missing_root = tempdir(CLEANUP => 1);
eval { new_ctl($env, root_dir => $missing_root, state_dir => File::Spec->catdir($missing_root, 'state')) };
like($@, qr/missing register defines file .*sw\/reg_defines_v7\.h.*sw\/reg_defines_v5\.h/, 'constructor fails clearly when no sw register defines file is present');
