...
│              reg_defines_v5.h  ←── 地址真值源                        │
│                         ↓ reg_req 总线                               │
│                   NetFPGA 1G 硬件寄存器窗口                           │
├──────────────────────────────────────────────────────────────────────┤
│              板级调试工具 (独立运行，不依赖上层编译链)                 │
│   board_debug/send_ann_offload.py    — 发 ANN EtherType 帧           │
│   board_debug/recv_ann_result.py     — 收 result 帧                  │
│   board_debug/run_ann_model_batch.py — 批量测试 + 对比 golden        │
│   (底层: board_debug/ann_packets.py  — 协议构造/解析)                │
└──────────────────────────────────────────────────────────────────────┘
```
...
这是软件侧寄存器地址的**唯一真值源**。`annctl` 启动时自动读取此文件来解析
`USER_TOP_*` 符号名。
...
## 8. mnist_toolchain — Binary-MNIST 训练导出
...
稳定回归 fixture 位于 `sw/testdata/mnist_binary_01/fixture_model.json`，
不需要 PyTorch 即可运行。
...
### 9.1 send_ann_offload.py — 发送 ANN offload 帧
...
# --src-port                 期望的 module header 源端口 (默认 0x0001)
# --dst-port-mask            期望的 module header 目的端口掩码 (默认 0x0008)
...
EtherType  : 0x88b5
task magic : 0xa11e
dst MAC    : 0b:ad:c0:de:00:01
src MAC    : f0:0d:ca:fe:00:02
```

### 9.2 recv_ann_result.py — 接收 ANN result 帧
...
sudo python sw/board_debug/run_ann_model_batch.py \
  ...
  --observed-out /tmp/ann_bundle/observed_results.json \
...
| 脚本 | 覆盖范围 | 依赖 |
|------|----------|------|
| `bash scripts/verify_unit_sw.sh` | CPU/GPU toolchain 各模块 Python 单元测试 | python3 |
| `bash scripts/verify_sw.sh` | annctl mocked 寄存器访问 + 软件集成流 | python3, perl |
| `bash scripts/verify_tb.sh` | RTL 仿真回归（Icarus Verilog） | iverilog |
| `bash scripts/verify_mnist_flow.sh` | MNIST fixture bundle + batch evaluator 接线 | python3 |

**建议每次修改工具链后先跑：**

```bash
bash scripts/verify_unit_sw.sh && bash scripts/verify_sw.sh
```
