...
python3 - "$ROOT_DIR/sw/reg_defines_v5.h" "$MOCK_STATE" <<'PY'
import json
import os
import re
import sys
...
BOARD_PACKET_JSON="$(python3 "$ROOT_DIR/sw/board_debug/send_ann_offload.py" --dump-json --src-port 0x1234 --dst-port-mask 0x0002)"
assert_contains "$BOARD_PACKET_JSON" "\"ethertype\": \"0x88b5\"" "board ANN frame builder ethertype mismatch"
assert_contains "$BOARD_PACKET_JSON" "\"task_magic\": \"0xa11e\"" "board ANN frame builder task magic mismatch"
assert_contains "$BOARD_PACKET_JSON" "\"expected_module_header\": \"0x0002000512340028\"" "board ANN frame builder module header mismatch"
assert_contains "$BOARD_PACKET_JSON" "\"wire_frame_hex\": \"0badc0de0001f00dcafe000288b5a11e1234000800000004fffb0006fff90008fff7000afff5\"" "board ANN frame builder wire frame mismatch"
...
bash "$ROOT_DIR/scripts/verify_mnist_flow.sh"
