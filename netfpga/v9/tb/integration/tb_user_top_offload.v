...
module tb_user_top_offload;
  ...
  localparam integer CPU_PROG_DEPTH = 14;
  ...
  localparam [3:0] COMPUTE_ST_WAIT_GPU      = 4'd8;
  localparam [3:0] COMPUTE_ST_CAPTURE_REQ   = 4'd9;
  localparam [3:0] COMPUTE_ST_CAPTURE_RESP  = 4'd10;
  localparam integer ACT_BASE_ADDR = 128;
  localparam integer WGT_BASE_ADDR = 3072;
  localparam integer BIAS_BASE_ADDR = 32;
  localparam integer Y_OFF = 800;
  localparam [15:0] DEFAULT_CAPTURE_BASE = ACT_BASE_ADDR + Y_OFF;
  localparam [31:0] ENGINE_CTRL_ENABLE = 32'h0000_0001;
  localparam [31:0] ENGINE_CTRL_COMPACT = 32'h0000_0002;
  localparam [31:0] ENGINE_CTRL_COUNT1 = 32'h0000_0100;
  localparam [31:0] ENGINE_CTRL_BASE_OVERRIDE = {DEFAULT_CAPTURE_BASE, 16'h0000};
  localparam [31:0] ENGINE_CTRL_BASE_ONLY = ENGINE_CTRL_BASE_OVERRIDE | ENGINE_CTRL_ENABLE;
  localparam [31:0] ENGINE_CTRL_BASE_COMPACT_COUNT1 = ENGINE_CTRL_BASE_OVERRIDE |
                                                      ENGINE_CTRL_COUNT1 |
                                                      ENGINE_CTRL_COMPACT |
                                                      ENGINE_CTRL_ENABLE;
  ...
  function [31:0] arm_mov_imm;
    ...
    input [11:0] imm12;
    begin
      arm_mov_imm = 32'hE3A00000 | (rd << 12) | imm12;
    end
  endfunction

  function [31:0] arm_lsl_imm;
    input [3:0] rd;
    input [3:0] rm;
    input [4:0] shamt;
    begin
      arm_lsl_imm = 32'hE1A00000 | (rd << 12) | (shamt << 7) | rm;
    end
  endfunction
  ...
  task prepare_ann_programs;
    ...
    begin
      ...
      cpu_program[3]  = arm_mov_imm(4'd2,  ACT_BASE_ADDR[11:0]);
      cpu_program[4]  = arm_mov_imm(4'd3,  12'd1536);
      cpu_program[5]  = arm_lsl_imm(4'd3,  4'd3, 5'd1);
      cpu_program[6]  = arm_mov_imm(4'd4,  BIAS_BASE_ADDR[11:0]);
      cpu_program[7]  = arm_mov_imm(4'd5,  12'd0);
      cpu_program[8]  = arm_str_imm(4'd0,  4'd10, 12'd8);
      cpu_program[9]  = arm_str_imm(4'd1,  4'd10, 12'd16);
      cpu_program[10] = arm_str_imm(4'd2,  4'd10, 12'd32);
      cpu_program[11] = arm_str_imm(4'd3,  4'd10, 12'd40);
      cpu_program[12] = arm_str_imm(4'd4,  4'd10, 12'd48);
      cpu_program[13] = arm_str_imm(4'd5,  4'd10, 12'd56);
  ...
  task init_ann_engine;
    ...
    begin
      ...
      write_engine_ctrl(ENGINE_CTRL_ENABLE);
      drive_idle_cycles(8);
  ...
  task expect_engine_ready;
    reg [31:0] engine_status;
    begin
      read_pipeline_reg(REG_HW_ENGINE_STATUS, engine_status);
      if (engine_status[0] !== 1'b1)
        record_failure("ANN engine unexpectedly lost ready");
    end
  endtask

  task expect_gpu_param_word;
    input integer addr;
    input [63:0] expected_word;
    begin
      if (dut.ann_engine.compute_core.u_shared_dmem.mem[addr] !== expected_word) begin
        $display("[%0t] DEBUG %0s param_addr=%0d exp=%016h act=%016h",
                 $time,
                 current_test_name,
                 addr,
                 expected_word,
                 dut.ann_engine.compute_core.u_shared_dmem.mem[addr]);
        record_failure("GPU param memory mismatch after rewrite");
      end
    end
  endtask

  task wait_for_gpu_busy;
    input integer timeout_cycles;
    integer waited;
    begin : wait_block
      waited = 0;
      while (dut.ann_engine.compute_core.gpu_busy !== 1'b1) begin
        @(posedge clk);
        waited = waited + 1;
        if (waited > timeout_cycles) begin
          record_failure("Timed out waiting for GPU busy");
          disable wait_block;
        end
      end
    end
  ...
  task wait_for_capture_phase;
    input integer timeout_cycles;
    integer waited;
    begin : wait_block
      waited = 0;
      while ((dut.ann_engine.compute_core.state != COMPUTE_ST_WAIT_GPU) &&
             (dut.ann_engine.compute_core.state != COMPUTE_ST_CAPTURE_REQ) &&
             (dut.ann_engine.compute_core.state != COMPUTE_ST_CAPTURE_RESP)) begin
        @(posedge clk);
        waited = waited + 1;
        if (waited > timeout_cycles) begin
          record_failure("Timed out waiting for capture phase");
          disable wait_block;
        end
      end
    end
  ...
  task build_ann_udp_packet_ex;
    ...
    begin
      ...
      payload_len = 8 + (emitted_feature_count * 2);
      ...
      bias0 = $signed(ann_bias_values[0]);
      bias1 = $signed(ann_bias_values[1]);
      ann_expected_sum = bias0;
      ann_expected_positive_count = bias1;
      for (i = 0; i < ANN_IN_DIM; i = i + 1) begin
        ann_expected_sum = ann_expected_sum +
                           $signed(ann_feature_values[i]) * $signed(ann_weight_values[i]);
        ann_expected_positive_count = ann_expected_positive_count +
                                      $signed(ann_feature_values[i]) *
                                      $signed(ann_weight_values[ANN_IN_DIM + i]);
      end
  ...
  task expect_ann_result_packet_words;
    ...
    begin
      if (tx_word_count <= RESULT_WORD1_TX_IDX) begin
        record_failure("ANN packet too short for in-place result rewrite");
      end
      else begin
        result_word_0 = {
  ...
  task expect_ann_result_packet;
    ...
    input [7:0] exp_status;
    begin
      expect_ann_result_packet_words(
        exp_src_port,
        exp_dst_port_mask,
        exp_status,
        ann_expected_sum[15:0],
        ann_expected_positive_count[15:0]
      );
    end
  endtask
  ...
  task expect_ann_compact_result_packet;
    ...
    input [15:0] exp_dst_port_mask;
    input [7:0] exp_status;
    input integer capture_count;
    reg [15:0] exp_class;
    reg signed [15:0] value0;
    reg signed [15:0] value1;
    reg signed [15:0] exp_value;
    begin
      value0 = ann_expected_sum[15:0];
      value1 = ann_expected_positive_count[15:0];
      if (capture_count <= 1) begin
        exp_class = 16'd0;
        exp_value = value0;
      end
      else if (value1 > value0) begin
        exp_class = 16'd1;
        exp_value = value1;
      end
      else begin
      ...
      if (out_wr && out_rdy) begin
        if (observed_words >= expected_words) begin
          ...
          if (out_data !== expected_data[observed_words]) begin
            $display("[%0t] DEBUG %0s word=%0d exp_data=%016h act_data=%016h exp_ctrl=%02h act_ctrl=%02h build_status=%02h compute_status=%02h result0=%04h result1=%04h parse_count=%0d base_a=%0d base_b=%0d base_c=%0d base_d=%0d work=%0d dmem_res0=%016h dmem_res1=%016h",
                     $time,
                     current_test_name,
                     observed_words,
                     ...
                     out_ctrl,
                     dut.ann_engine.build_status,
                     dut.ann_engine.compute_result_status,
                     dut.ann_engine.compute_result_data_0,
                     dut.ann_engine.compute_result_data_1,
                     dut.ann_engine.parse_parsed_feature_count,
                     dut.ann_engine.compute_core.gpu_core.u_ctrl.base_a,
                     dut.ann_engine.compute_core.gpu_core.u_ctrl.base_b,
                     dut.ann_engine.compute_core.gpu_core.u_ctrl.base_c,
                     dut.ann_engine.compute_core.gpu_core.u_ctrl.base_d,
                     dut.ann_engine.compute_core.gpu_core.u_ctrl.work_size_eff,
                     dut.ann_engine.compute_core.u_shared_dmem.mem[ACT_BASE_ADDR + Y_OFF],
                     dut.ann_engine.compute_core.u_shared_dmem.mem[ACT_BASE_ADDR + Y_OFF + 1]);
...
`endif
    ...
    auto_expect_tx_words = 1'b1;
    build_ann_udp_packet_ex(4, 4, 16'h0001, 16'h0008, 16'h9abc, 2, 16'hbeef, 16'h0000, ANN_UDP_DST_PORT);
    send_packet(-1, 0, 1);
    finish_test;

    start_test("tc04_ann_truncated_payload_status");
    auto_expect_tx_words = 1'b0;
    build_ann_udp_packet_ex(8, 5, 16'h0002, 16'h0010, 16'h2468, 4, ANN_TASK_MAGIC, 16'h0000, ANN_UDP_DST_PORT);
    ...
    bp_mode = BP_HEAVY;
    auto_expect_tx_words = 1'b0;
    build_ann_udp_packet(8, 16'h0003, 16'h0002, 16'h55aa, 6);
    ...
    auto_expect_tx_words = 1'b0;
    build_ann_udp_packet(8, 16'h0001, 16'h0008, 16'h1357, 7);
    expect_ann_result_packet(16'h0001, 16'h0008, ANN_STATUS_OK);
    send_packet(-1, 0, 1);
    expect_cpu_replay_matches_tx(TEST_TIMEOUT_CYCLES);
    build_ann_udp_packet(8, 16'h0002, 16'h0010, 16'h2468, 8);
    expect_ann_result_packet(16'h0002, 16'h0010, ANN_STATUS_OK);
    send_packet(-1, 0, 1);
    expect_cpu_replay_matches_tx(TEST_TIMEOUT_CYCLES);
    finish_test;

    start_test("tc07_ann_param_rewrite_staging");
    write_gpu_param_word(WGT_BASE_ADDR + 0, {48'd0, 16'sd4});
    ann_weight_values[0] = 16'sd4;
    write_gpu_param_word(WGT_BASE_ADDR + 1, {48'd0, -16'sd3});
    ann_weight_values[1] = -16'sd3;
    write_gpu_param_word(WGT_BASE_ADDR + 0, {48'd0, 16'sd7});
    ann_weight_values[0] = 16'sd7;
    drive_idle_cycles(6);
    expect_engine_ready;
    if (dut.ann_engine.compute_core.pending_param_valid !== 1'b0)
      record_failure("Pending GPU param write did not drain");
    expect_gpu_param_word(WGT_BASE_ADDR + 0, {48'd0, ann_weight_values[0]});
    expect_gpu_param_word(WGT_BASE_ADDR + 1, {48'd0, ann_weight_values[1]});
    auto_expect_tx_words = 1'b0;
    build_ann_udp_packet(8, 16'h0004, 16'h0001, 16'h7777, 9);
    expect_ann_result_packet(16'h0004, 16'h0001, ANN_STATUS_OK);
    send_packet(-1, 0, 1);
    expect_cpu_replay_matches_tx(TEST_TIMEOUT_CYCLES);
    finish_test;

    write_engine_ctrl(ENGINE_CTRL_BASE_ONLY);
    drive_idle_cycles(2);
    expect_engine_ready;
    start_test("tc08_ann_engine_ctrl_base_override");
    auto_expect_tx_words = 1'b0;
    build_ann_udp_packet(8, 16'h0005, 16'h0004, 16'h8888, 10);
    expect_ann_result_packet(16'h0005, 16'h0004, ANN_STATUS_OK);
    send_packet(-1, 0, 1);
    expect_cpu_replay_matches_tx(TEST_TIMEOUT_CYCLES);
    wait_for_capture_phase(TEST_TIMEOUT_CYCLES);
    if (dut.sw_engine_ctrl_latch !== ENGINE_CTRL_BASE_ONLY)
      record_failure("Engine control latch missed base override write");
    if (dut.ann_engine.compute_core.cfg_capture_base_addr !== DEFAULT_CAPTURE_BASE[13:0])
      record_failure("Capture base config did not latch override");
    finish_test;

    write_engine_ctrl(ENGINE_CTRL_ENABLE);
    drive_idle_cycles(2);
    expect_engine_ready;
    start_test("tc09_ann_engine_ctrl_update_deferred");
    auto_expect_tx_words = 1'b0;
    build_ann_udp_packet(8, 16'h0006, 16'h0002, 16'h9999, 11);
    expect_ann_result_packet(16'h0006, 16'h0002, ANN_STATUS_OK);
    send_packet(-1, 0, 1);
    wait_for_gpu_busy(TEST_TIMEOUT_CYCLES);
    write_engine_ctrl(ENGINE_CTRL_BASE_COMPACT_COUNT1);
    drive_idle_cycles(2);
    if (dut.sw_engine_ctrl_latch !== ENGINE_CTRL_BASE_COMPACT_COUNT1)
      record_failure("Engine control latch did not update during in-flight packet");
    if (dut.ann_engine.compute_core.cfg_compact_result_enable !== 1'b0)
      record_failure("In-flight packet observed updated compact flag");
    finish_test;

    expect_engine_ready;
    start_test("tc10_ann_engine_ctrl_next_packet_uses_new_cfg");
    auto_expect_tx_words = 1'b0;
    build_ann_udp_packet(8, 16'h0007, 16'h0008, 16'haaaa, 12);
    expect_ann_compact_result_packet(16'h0007, 16'h0008, ANN_STATUS_OK, 1);
    send_packet(-1, 0, 1);
    expect_cpu_replay_matches_tx(TEST_TIMEOUT_CYCLES);
    wait_for_capture_phase(TEST_TIMEOUT_CYCLES);
    if (dut.ann_engine.compute_core.cfg_compact_result_enable !== 1'b1)
      record_failure("Next packet did not latch compact mode");
    if (dut.ann_engine.compute_core.cfg_capture_last_index !== 16'd0)
      record_failure("Next packet did not latch explicit capture count");
    if (dut.ann_engine.compute_core.cfg_capture_base_addr !== DEFAULT_CAPTURE_BASE[13:0])
      record_failure("Next packet did not latch capture base override");
    finish_test;
