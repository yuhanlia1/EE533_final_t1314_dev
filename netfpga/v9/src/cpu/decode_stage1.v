module decode_stage1 #(
  ...
  // When WB drives the same register in the same cycle that ID reads it,
  // bypass the write data around the synchronous regfile read port.
  assign id1_R1_out = (rf_we && (r0addr == rf_waddr)) ? rf_wdata : raw_R1_out;
  assign id1_R2_out = (rf_we && (r1addr == rf_waddr)) ? rf_wdata : raw_R2_out;
