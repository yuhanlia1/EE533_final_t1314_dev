...
class SweepRunner:
    ...
    def _single_packet_bypass_verdict(self, manifest, capture_state, variant):
        ...
        expected_magic = "0x%04x" % (DEFAULT_WRONG_MAGIC if variant == "wrong_magic" else ANN_TASK_MAGIC)
        expected_udp_dst = (
            manifest["network"]["dst_udp_port"]
            if variant == "wrong_magic"
            else "0x%04x" % DEFAULT_WRONG_PORT
        )
        passed = (
            len(frames) >= 1
    ...
    def _execute_batch_workload(self, run_dir, manifest, handle, workload):
        ...
        diagnostics = self._run_capture_sequence(run_dir, manifest, handle)
    ...
    def _run_capture_sequence(self, run_dir, manifest, handle):
        commands_dir = run_dir / "commands"
